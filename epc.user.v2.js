...
function getUrlArgs(){
	var argStr = location.search.replace('?','').split('&');
	var argObj = {};
	for(var i in argStr){
		var tmp = argStr[i].split('=');
		if(tmp[0]=='') continue;
		argObj[ tmp[0] ] = tmp[1];
	}
	return argObj;
...
function createUrl(obj){
	var path = location.href.substr(0,location.href.indexOf('?')+1);
	var argStr = [];
	for(var i in obj){
		argStr.push ( i + '=' + obj[i] );
	}
	location.href = path + argStr.join('&');
}
var config = new Config();
var allPage ;
function getCurrentInfo(){
	if(typeof(localStorage['firstWeek'])=='undefined'){
		var d = new Date('2013-09-02 00:00:01').getTime();
	}else{
		var d =  new Date(localStorage['firstWeek']).getTime();
	}
	var cd = new Date().getTime();
	var cweek = Math.floor((cd-d)/3600000/24/7);
...
$(document).bind('next',function(){
	...
	var url = location.href;
	if(week==null || c.startWeek > cweek){
		url = url + '&week='+c.startWeek;
		week=['week='+c.startWeek];
	}
	if(day==null || c.startWeek > cweek){
